...
test('ページと共有用metaが正しく表示される', async ({ page }) => {
  await expect(page.getByRole('heading', { name: '推し活プロフィールメーカー' })).toBeVisible();
  await expect(page).toHaveTitle('推し活プロフィールメーカー ～Thanks, Chelsea!～');
  ...
  await expect(page.getByText('入力内容はこの端末のブラウザ内だけに自動保存され')).toBeVisible();
});
...
test('入力内容を端末内へ保存し、再読み込み後に復元する', async ({ page }) => {
  ...
  await page.getByLabel('推しになったきっかけ').fill('ライブで一目惚れしました');
  ...
  await expect(page.getByLabel('名前', { exact: true })).toHaveValue('テストユーザー');
  await expect(page.getByLabel('誕生月')).toHaveValue('12');
  await expect(page.getByLabel('誕生日の日')).toHaveValue('31');

  await page.getByRole('tab', { name: '右ページ' }).click();
  await expect(page.getByLabel('推しへひとこと♡')).toHaveValue('いつもありがとう！');
  await expect(
...
test('プレビューを表示してPNG画像を保存できる', async ({ page }) => {
  ...
  const canvas = page.getByLabel('入力内容を反映した推し活プロフィールのプレビュー');
